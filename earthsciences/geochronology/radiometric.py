...
def isochron_dating(
    parent_ratios: np.ndarray,
    daughter_ratios: np.ndarray,
...
def concordia_diagram_age(
    ratio_206_238: np.ndarray,
    ratio_207_235: np.ndarray,
    uncertainty_206_238: np.ndarray | None = None,
    uncertainty_207_235: np.ndarray | None = None,
) -> dict:
    """
...
def fission_track_age(
    track_density_sample: float, track_density_standard: float, age_standard: float, zeta: float
) -> tuple[float, float]:
    """
    ...
    # Age calculation
    age = (1 / lambda_d) * np.log(
        1 + lambda_d * zeta * track_density_sample / track_density_standard
    )
...
def cosmogenic_exposure_age(
    nuclide_concentration: float, production_rate: float, decay_constant: float = 0.0
) -> float:
    """
    ...
    if decay_constant == 0:
        ...
        age = -(1 / decay_constant) * np.log(
            1 - decay_constant * nuclide_concentration / production_rate
        )
