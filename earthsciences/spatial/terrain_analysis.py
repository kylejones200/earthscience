...
def slope_aspect(dem: np.ndarray,
                cell_size: float = 1.0,
...
def curvature(dem: np.ndarray,
    ...
    # Mean curvature
    with np.errstate(divide='ignore', invalid='ignore'):
        mean_curv = -((1 + q**2) * d2z_dx2 - 2*p*q*d2z_dxdy + 
                      (1 + p**2) * d2z_dy2) / (2 * (1 + p2_q2)**1.5)
...
def topographic_wetness_index(dem: np.ndarray,
                              cell_size: float = 1.0) -> np.ndarray:
    """
...
def calculate_flow_accumulation(dem: np.ndarray,
                               cell_size: float = 1.0) -> np.ndarray:
    """
...
def stream_power_index(dem: np.ndarray,
                      cell_size: float = 1.0) -> np.ndarray:
    """
...
def terrain_ruggedness_index(dem: np.ndarray,
...
def topographic_position_index(dem: np.ndarray,
                               inner_radius: int = 1,
                               outer_radius: int = 5) -> np.ndarray:
    """
