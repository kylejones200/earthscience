...
def compute_variogram(x: np.ndarray,
                     y: np.ndarray,
                     values: np.ndarray,
                     n_lags: int = 15,
                     max_lag: Optional[float] = None) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
...
def variogram_cloud(x: np.ndarray,
                   y: np.ndarray,
                   values: np.ndarray,
...
def spherical_model(h: np.ndarray, nugget: float, sill: float,
                   range_param: float) -> np.ndarray:
    """
...
def exponential_model(h: np.ndarray, nugget: float, sill: float,
                     range_param: float) -> np.ndarray:
    """
    ...
    mask = h > 0
    gamma[mask] = nugget + (sill - nugget) * (
        1 - np.exp(-3 * h[mask] / range_param)
    )
    
    return gamma


def gaussian_model(h: np.ndarray, nugget: float, sill: float,
                  range_param: float) -> np.ndarray:
    """
    ...
    mask = h > 0
    gamma[mask] = nugget + (sill - nugget) * (
        1 - np.exp(-3 * (h[mask] / range_param)**2)
    )
    
    return gamma


def fit_variogram_model(lag_dist: np.ndarray,
                       gamma: np.ndarray,
                       model: str = 'spherical',
                       n_pairs: Optional[np.ndarray] = None) -> Dict:
    """
    ...
    dict
        ...
        - function: fitted variogram function
    ...
    # Bounds
    bounds = ([0, nugget_init, 0], 
              [sill_init, sill_init * 2, np.max(lag_dist) * 2])
    ...
    # Fit model
    try:
        popt, pcov = curve_fit(model_func, lag_dist, gamma, 
        ...
        warnings.warn(
            f"Variogram model fitting failed: {e}. "
            f"Using initial parameter estimates instead.",
            RuntimeWarning
        )
        nugget = nugget_init
        sill = sill_init
...
def anisotropic_variogram(x: np.ndarray,
                         y: np.ndarray,
                         values: np.ndarray,
                         n_directions: int = 4,
                         tolerance: float = 22.5) -> Dict:
    """
