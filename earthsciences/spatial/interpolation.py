...
def idw_interpolation(x: np.ndarray,
                     y: np.ndarray,
                     values: np.ndarray,
                     grid_x: np.ndarray,
                     grid_y: np.ndarray,
    ...
    for i, query in enumerate(query_points):
        if radius is None:
            ...
            if len(indices) < min_points:
                ...
                distances = np.sqrt((x[indices] - query[0])**2 + 
                                   (y[indices] - query[1])**2)
                mask = distances > 0
...
def nearest_neighbor(x: np.ndarray,
                    y: np.ndarray,
                    values: np.ndarray,
                    grid_x: np.ndarray,
...
def griddata_interpolation(x: np.ndarray,
                          y: np.ndarray,
                          values: np.ndarray,
                          grid_x: np.ndarray,
                          grid_y: np.ndarray,
...
def natural_neighbor(x: np.ndarray,
                    y: np.ndarray,
                    values: np.ndarray,
                    grid_x: np.ndarray,
                    grid_y: np.ndarray) -> np.ndarray:
...
def bilinear_interpolation(x: np.ndarray,
                          y: np.ndarray,
                          values: np.ndarray,
                          query_x: float,
                          query_y: float) -> float:
