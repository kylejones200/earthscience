...
def ordinary_kriging(x: np.ndarray,
                    y: np.ndarray,
                    values: np.ndarray,
                    grid_x: np.ndarray,
                    grid_y: np.ndarray,
                    variogram_func: Callable,
                    return_variance: bool = False) -> np.ndarray:
    """
...
def simple_kriging(x: np.ndarray,
                  y: np.ndarray,
                  values: np.ndarray,
                  grid_x: np.ndarray,
                  grid_y: np.ndarray,
...
def universal_kriging(x: np.ndarray,
                     y: np.ndarray,
                     values: np.ndarray,
                     grid_x: np.ndarray,
                     grid_y: np.ndarray,
                     variogram_func: Callable,
                     drift_order: int = 1) -> np.ndarray:
    """
    ...
    if drift_order == 1:
        ...
        F_grid = np.column_stack([
            np.ones(n_grid), grid_x_flat, grid_y_flat,
            grid_x_flat**2, grid_x_flat*grid_y_flat, grid_y_flat**2
        ])  # (n_grid, 6)
...
def kriging_variance(x: np.ndarray,
                    y: np.ndarray,
                    grid_x: np.ndarray,
                    grid_y: np.ndarray,
                    variogram_func: Callable) -> np.ndarray:
    """
    ...
    _, variance = ordinary_kriging(x, y, values, grid_x, grid_y,
