...
def histogram_equalization(image: np.ndarray, n_bins: int = 256) -> np.ndarray:
    """
...
def contrast_stretch(
    image: np.ndarray, lower_percentile: float = 2.0, upper_percentile: float = 98.0
) -> np.ndarray:
    """
...
def adaptive_histogram_equalization(
    image: np.ndarray, clip_limit: float = 0.03, tile_size: tuple[int, int] = (8, 8)
) -> np.ndarray:
    """
    ...
    try:
        ...
        enhanced = exposure.equalize_adapthist(
            normalized, clip_limit=clip_limit, kernel_size=tile_size
        )
        ...
        case "bilateral":
            try:
                ...
                return denoise_bilateral(
                    image, sigma_spatial=sigma_spatial, sigma_color=sigma_color
                )
            except ImportError:
...
def remove_periodic_noise(
    image: np.ndarray, frequency_threshold: float | None = None
) -> np.ndarray:
    """
...
def atmospheric_correction_dos(
    image: np.ndarray, dark_object_value: float | None = None
) -> np.ndarray:
    """
...
def pan_sharpen(pan: np.ndarray, multispectral: np.ndarray, method: str = "brovey") -> np.ndarray:
    """
...
def cloud_mask(red: np.ndarray, nir: np.ndarray, threshold: float = 0.3) -> np.ndarray:
    """
