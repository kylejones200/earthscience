...
def shape_analysis(binary_image: np.ndarray, min_size: int = 10) -> dict:
    """
...
def fabric_analysis(orientations: np.ndarray, weights: np.ndarray | None = None) -> dict:
    """
    ...
    # Orientation tensor
    tensor = np.array([[S_xx, S_xy], [S_xy, S_yy]])
...
def watershed_segmentation(
    image: np.ndarray, min_distance: int = 10, threshold_rel: float = 0.5
) -> dict:
    """
    ...
    coords = feature.peak_local_max(
        distance, min_distance=min_distance, threshold_rel=threshold_rel, labels=binary
    )
...
def detect_circular_objects(
    image: np.ndarray, min_radius: int = 10, max_radius: int = 100, sensitivity: float = 0.5
) -> dict:
    """
