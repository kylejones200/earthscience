...
def ndvi(nir: np.ndarray, red: np.ndarray,
         mask_invalid: bool = True) -> np.ndarray:
    """
    ...
    if mask_invalid:
        ...
        ndvi_values = np.where((ndvi_values >= -1) & (ndvi_values <= 1),
                               ndvi_values, np.nan)
...
def savi(nir: np.ndarray, red: np.ndarray, L: float = 0.5,
         mask_invalid: bool = True) -> np.ndarray:
    """
...
def ndwi(nir: np.ndarray, swir: np.ndarray,
         mask_invalid: bool = True) -> np.ndarray:
    """
    ...
    if mask_invalid:
        ...
        ndwi_values = np.where((ndwi_values >= -1) & (ndwi_values <= 1),
                               ndwi_values, np.nan)
    
    return ndwi_values


def ndbi(swir: np.ndarray, nir: np.ndarray,
         mask_invalid: bool = True) -> np.ndarray:
    """
...
def nbr(nir: np.ndarray, swir: np.ndarray,
        mask_invalid: bool = True) -> np.ndarray:
    """
...
def gndvi(nir: np.ndarray, green: np.ndarray,
          mask_invalid: bool = True) -> np.ndarray:
    """
...
def calculate_toa_reflectance(dn: np.ndarray,
                               radiance_mult: float,
                               radiance_add: float,
                               sun_elevation: float,
                               esun: float,
                               d: float = 1.0) -> np.ndarray:
    """
