...
def magnitude_calculation(
    ...
    station_correction: np.ndarray | None = None,
) -> dict:
    """
...
def epicenter_location(
    ...
    def residual(params):
        ...
        residuals = []
        for i, (t_obs, station, phase) in enumerate(
            zip(arrival_times, station_coords, phase_types)
        ):
...
def focal_mechanism(
    first_motions: np.ndarray, azimuths: np.ndarray, takeoff_angles: np.ndarray
) -> dict:
    """
