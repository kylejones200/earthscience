...
def cross_validation_classification(
    X: np.ndarray, y: np.ndarray, model, k_folds: int = 5, random_state: int | None = None
) -> dict:
    """
...
def mahalanobis_distance(
    X: np.ndarray, center: np.ndarray | None = None, cov: np.ndarray | None = None
) -> np.ndarray:
    """
...
def roc_curve_analysis(y_true: np.ndarray, y_proba: np.ndarray, pos_label: int = 1) -> dict:
    """
