...
def umap_embedding(data: np.ndarray,
                  n_components: int = 2,
                  n_neighbors: int = 15,
                  min_dist: float = 0.1,
...
def mds(data: np.ndarray,
    ...
    # Perform MDS
    mds_model = MDS(n_components=n_components, metric=metric, 
                   max_iter=max_iter, dissimilarity='precomputed' 
                   if data.shape[0] == data.shape[1] else 'euclidean')
...
def lle(data: np.ndarray,
    ...
    # Perform LLE
    lle_model = LocallyLinearEmbedding(n_components=n_components,
                                       n_neighbors=n_neighbors,
                                       method=method)
...
def spectral_embedding(data: np.ndarray,
                      n_components: int = 2,
...
def compare_embeddings(data: np.ndarray,
                      methods: list = ['pca', 'tsne', 'isomap'],
