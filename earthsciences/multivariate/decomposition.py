...
def independent_component_analysis(
    ...
    # Perform ICA
    ica = FastICA(n_components=n_components, max_iter=max_iter, random_state=random_state)
...
def truncated_svd(data: np.ndarray, n_components: int = 10, algorithm: str = "randomized") -> dict:
    """
...
def sparse_pca(
    data: np.ndarray, n_components: int = 10, alpha: float = 1.0, max_iter: int = 100
) -> dict:
    """
...
def canonical_correlation_analysis(X: np.ndarray, Y: np.ndarray, n_components: int = 2) -> dict:
    """
    ...
    correlations = np.array([np.corrcoef(X_c[:, i], Y_c[:, i])[0, 1] for i in range(n_components)])
...
def kernel_pca(
    data: np.ndarray, n_components: int = 2, kernel: str = "rbf", gamma: float | None = None
) -> dict:
    """
...
def dictionary_learning(
    ...
    dl = DictionaryLearning(n_components=n_components, alpha=alpha, max_iter=max_iter)
