...
def principal_component_analysis(
    data: np.ndarray, n_components: int | None = None, standardize: bool = True
) -> dict:
    """
...
def pca_biplot(
    data: np.ndarray, feature_names: list | None = None, pc1: int = 0, pc2: int = 1
) -> dict:
    """
...
def factor_analysis(data: np.ndarray, n_factors: int = 2) -> dict:
    """
...
def rotation_varimax(
    loadings: np.ndarray, max_iter: int = 100, tol: float = 1e-6
    ...
    for _ in range(max_iter):
        ...
        u, _, vt = svd(
            loadings.T
            @ (normalized * rotated**2 - (np.sum(normalized * rotated**2, axis=0) / n_vars))
        )
