...
def generate_template(
    output_path: str | Path = "analysis_config.yaml", template_type: str = "full"
):
    """
    ...
    if template_type not in templates:
        raise ValueError(
            f"Unknown template type: {template_type}. " f"Choose from: {list(templates.keys())}"
        )
    ...
    logger.info(f"  Edit the file and run: earthsciences run --config {output_path}")
...
def _minimal_template() -> dict:
    ...
    return {
        "project": {"name": "Quick Analysis", "output_dir": "./results"},
        "data": {
            "input_file": "data/my_data.csv",
            "x_column": "x",
            "y_column": "y",
        ...
        "kriging": {
            "grid": {"x_min": 0.0, "x_max": 100.0, "y_min": 0.0, "y_max": 100.0, "resolution": 1.0}
        },
    }
...
def _kriging_template() -> dict:
    ...
    return {
        "project": {"name": "Kriging Analysis", "output_dir": "./kriging_results"},
        "data": {
        ...
        "variogram": {"n_lags": 15, "models": ["spherical", "exponential"], "auto_fit": True},
        "kriging": {
            "method": "ordinary",
            "neighborhood": {"max_neighbors": 25, "min_neighbors": 5, "search_radius": 50.0},
            "grid": {"x_min": 0.0, "x_max": 100.0, "y_min": 0.0, "y_max": 100.0, "resolution": 1.0},
            "compute_variance": True,
        },
        "visualization": {
...
def _validation_template() -> dict:
    ...
    return {
        "project": {"name": "Validation Study", "output_dir": "./validation_results"},
        "data": {
        ...
        "kriging": {
            "method": "ordinary",
            "grid": {"x_min": 0.0, "x_max": 100.0, "y_min": 0.0, "y_max": 100.0, "resolution": 2.0},
        },
        "validation": {
            "cross_validation": True,
            "n_folds": 10,
