...
class PreprocessingConfig(BaseModel):
    ...
    transform: Literal["log", "boxcox", "normal_score"] | None = Field(
        None, description="Data transformation"
    )
    handle_negatives: bool = Field(
        default=True, description="Shift data to handle negative values for log transform"
    )
...
class VariogramConfig(BaseModel):
    ...
    max_lag: float | None = Field(None, ge=0, description="Maximum lag distance")
    ...
    models: list[Literal["spherical", "exponential", "gaussian", "linear", "power"]] = Field(
        default=["spherical", "exponential"], description="Variogram models to try"
    )
    auto_fit: bool = Field(default=True, description="Automatic model fitting")
    fit_method: Literal["ols", "wls", "ml"] = Field(
        default="wls", description="Fitting method (OLS, WLS, ML)"
    )
    direction: float | None = Field(
        None, ge=0, le=360, description="Direction for anisotropy (degrees)"
    )
    tolerance: float | None = Field(
        None, ge=0, le=90, description="Angular tolerance for anisotropy"
    )
...
class NeighborhoodConfig(BaseModel):
    ...
    search_radius: float | None = Field(None, ge=0, description="Search radius")
    use_quadrant_search: bool = Field(default=False, description="Use quadrant-based search")
...
class KrigingConfig(BaseModel):
    ...
    method: Literal["ordinary", "simple", "universal"] = Field(
        default="ordinary", description="Kriging method"
    )
    drift_terms: list[str] | None = Field(
...
class ValidationConfig(BaseModel):
    ...
    method: Literal["kfold", "loo", "spatial"] = Field(
        default="kfold", description="Cross-validation method"
    )
    metrics: list[Literal["rmse", "mae", "r2", "mse", "mape"]] = Field(
        default=["rmse", "mae", "r2"], description="Validation metrics"
    )
    stratified: bool = Field(default=False, description="Use stratified sampling")
...
class VisualizationConfig(BaseModel):
    ...
    style: Literal["minimalist", "default", "seaborn"] = Field(
        default="minimalist", description="Plot style"
    )
    plots: list[
        Literal[
            "variogram",
        ...
        default=["variogram", "kriging_map", "cross_validation"], description="Plots to generate"
    )
    ...
    figsize: list[float] | None = Field(None, description="Figure size [width, height] in inches")
    contour_levels: int | None = Field(None, ge=5, le=50, description="Number of contour levels")
...
class OutputConfig(BaseModel):
    ...
    formats: list[Literal["png", "pdf", "geotiff", "csv", "npy"]] = Field(
        default=["png", "csv"], description="Output formats"
    )
