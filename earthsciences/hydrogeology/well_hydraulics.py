...
def darcy_velocity(
    K: float, hydraulic_gradient: float, porosity: float | None = None
...
def aquifer_safe_yield(
    recharge: float,
    area: float,
    current_pumping: float,
...
def groundwater_age_dating(
    tracer_concentration: float, initial_concentration: float, decay_constant: float
) -> float:
    """
    ...
    if tracer_concentration > initial_concentration:
        warnings.warn(
            "Measured concentration exceeds initial; may indicate contamination or mixing"
        )
