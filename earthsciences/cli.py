...
@click.option("--output", "-o", type=click.Path(), help="Override output directory from config")
...
@cli.command()
@click.option(
    "--output", "-o", default="analysis_config.yaml", type=click.Path(), help="Output file path"
)
@click.option(
    "--template",
    "-t",
...
def show(config_file):
    ...
    try:
        ...
        from earthsciences.config.parser import config_to_dict, load_config
...
@cli.command()
@click.argument("config_file", type=click.Path(exists=True))
@click.option("--output", "-o", default="config_summary.txt", help="Output summary file")
def summarize(config_file, output):
    ...
    try:
        ...
        with open(output, "w") as f:
            ...
            f.write(
                f"  Columns: X={config.data.x_column}, Y={config.data.y_column}, Z={config.data.z_column}\n\n"
            )
