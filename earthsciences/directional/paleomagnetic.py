...
def geographic_to_magnetic(declination: float,
                          inclination: float,
                          latitude: float,
    ...
    pole_lat_rad = np.arcsin(
        np.sin(lat_rad) * np.cos(p) + 
        np.cos(lat_rad) * np.sin(p) * np.cos(dec_rad)
    )
...
def virtual_geomagnetic_pole(declination: np.ndarray,
    ...
    from ..directional.spherical import spherical_mean, fisher_distribution
...
def paleomagnetic_pole_position(declination: float,
                                inclination: float) -> Tuple[float, float]:
    """
...
def inclination_shallowing_correction(observed_inclination: float,
                                     f: float = 0.6) -> float:
    """
...
def magnetic_reversal_detection(declination: np.ndarray,
                                inclination: np.ndarray,
                                threshold: float = 120.0) -> Dict:
    """
...
def paleomagnetic_secular_variation(declination: np.ndarray,
                                   inclination: np.ndarray,
                                   ages: np.ndarray) -> Dict:
    """
...
def demagnetization_analysis(temperatures: np.ndarray,
                            magnetization: np.ndarray) -> Dict:
    """
