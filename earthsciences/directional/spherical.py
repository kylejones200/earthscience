...
def spherical_mean(theta: np.ndarray, phi: np.ndarray,
...
def spherical_variance(theta: np.ndarray, phi: np.ndarray,
...
def fisher_distribution(theta: np.ndarray, phi: np.ndarray,
...
def kent_distribution(theta: np.ndarray, phi: np.ndarray,
                     degrees: bool = True) -> Dict:
    """
...
def stereonet_projection(theta: np.ndarray, phi: np.ndarray,
                        degrees: bool = True,
                        lower_hemisphere: bool = True) -> Tuple[np.ndarray, np.ndarray]:
    """
...
def resultant_length(theta: np.ndarray, phi: np.ndarray,
                    degrees: bool = True) -> float:
    """
...
def fisher_pdf(theta: float, phi: float,
              mean_theta: float, mean_phi: float,
              kappa: float) -> float:
    """
    ...
    """
    # Angular distance
    cos_dist = (np.cos(theta) * np.cos(mean_theta) +
                np.sin(theta) * np.sin(mean_theta) *
                np.cos(phi - mean_phi))
...
def fisher_rvs(mean_theta: float, mean_phi: float, kappa: float,
              size: int = 1) -> Tuple[np.ndarray, np.ndarray]:
    """
    ...
    while len(theta_samples) < size:
        ...
        cos_dist = (np.cos(theta_prop) * np.cos(mean_theta) +
                   np.sin(theta_prop) * np.sin(mean_theta) *
...
def spherical_kde(theta: np.ndarray, phi: np.ndarray,
                 bandwidth: float = 0.3) -> Dict:
    """
    ...
    def density(theta_eval, phi_eval):
        ...
        for t, p in zip(theta, phi):
            cos_dist = (np.cos(theta_eval) * np.cos(t) +
                       np.sin(theta_eval) * np.sin(t) *
                       np.cos(phi_eval - p))
            cos_dist = np.clip(cos_dist, -1, 1)
