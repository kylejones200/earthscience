...
def formation_factor(
    porosity: np.ndarray, cementation_exponent: float = 2.0, tortuosity_factor: float = 1.0
) -> np.ndarray:
    """
...
def archie_equation(
    Rt: np.ndarray, Rw: float, porosity: np.ndarray, m: float = 2.0, n: float = 2.0, a: float = 1.0
) -> np.ndarray:
    """
...
def water_saturation(
    Rt: np.ndarray, Rw: float, porosity: np.ndarray, method: str = "archie", **kwargs
) -> dict:
    """
...
def permeability_kozeny_carman(
    porosity: np.ndarray, grain_size: np.ndarray, tortuosity: float = 1.0
) -> np.ndarray:
    """
