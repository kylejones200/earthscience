...
def decline_curve_analysis(time: np.ndarray, production: np.ndarray,
                          method: str = 'arps',
                          forecast_time: Optional[np.ndarray] = None) -> Dict:
    """
    ...
    try:
        if method == 'exponential':
            popt, _ = curve_fit(exponential, time, production,
            ...
            popt, _ = curve_fit(harmonic, time, production,
                               p0=[production[0], 0.01],
                               bounds=([0, 0], [np.inf, 1]))
            qi, Di = popt
            b = 1
            ...
            popt, _ = curve_fit(hyperbolic, time, production,
    ...
    if b < 1:
        ...
        EUR = np.trapz(model(np.linspace(0, time[-1]*5, 1000)),
                      np.linspace(0, time[-1]*5, 1000))
...
def oil_formation_volume_factor(Rs: np.ndarray, gas_gravity: float,
                                oil_gravity: float, temperature: float) -> np.ndarray:
    """
...
def gas_deviation_factor(pressure: np.ndarray, temperature: float,
                         gas_gravity: float = 0.65) -> np.ndarray:
    """
