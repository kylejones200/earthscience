...
def fit_gev(data: np.ndarray, method: str = "mle") -> dict:
    """
...
def return_period(gev_params: dict, level: float) -> float:
    """
...
def fit_gpd(data: np.ndarray, threshold: float | None = None) -> dict:
    """
...
def mean_excess_plot(data: np.ndarray, thresholds: np.ndarray | None = None) -> dict:
    """
...
def block_maxima(data: np.ndarray, block_size: int, method: str = "max") -> np.ndarray:
    """
...
def confidence_intervals_return_level(
    gev_params: dict,
    return_period: float,
    data: np.ndarray,
...
def gev_return_level(params: dict, return_period: float) -> float:
    ...
    if abs(shape) < 1e-10:
        return_level = location - scale * np.log(-np.log(p))
    else:
        return_level = location - (scale / shape) * (1 - (-np.log(p)) ** (-shape))
...
def empirical_return_period(data: np.ndarray) -> dict:
    ...
    return {"values": sorted_data, "return_periods": return_periods}
...
def return_level_plot(data: np.ndarray, method: str = "gev") -> dict:
    ...
    if method == "gev":
        params = fit_gev(data)
        return_periods = np.logspace(np.log10(1.01), 2, 50)
...
def bootstrap_return_level(
    data: np.ndarray, return_period: float, n_bootstrap: int = 1000, confidence: float = 0.95
) -> dict:
    """
