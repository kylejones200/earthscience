...
def t_test(sample1: np.ndarray, 
    ...
    if sample2 is None:
        # One-sample t-test
        statistic, p_value = stats.ttest_1samp(sample1, mu, 
...
def f_test(sample1: np.ndarray, sample2: np.ndarray) -> Dict:
    ...
    # Two-tailed p-value
    p_value = 2 * min(stats.f.cdf(f_stat, dof1, dof2),
                      1 - stats.f.cdf(f_stat, dof1, dof2))
...
def mann_whitney_test(sample1: np.ndarray, 
                      sample2: np.ndarray,
                      alternative: str = 'two-sided') -> Dict:
    """
...
def correlation_significance(r: float, n: int,
...
def ansari_bradley_test(x: np.ndarray, y: np.ndarray,
