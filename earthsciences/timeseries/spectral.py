...
def power_spectrum(data: np.ndarray,
                  ...
                  sampling_rate: Optional[float] = None) -> Tuple[np.ndarray, np.ndarray]:
    """
    ...
    match method:
        ...
        case _:
            raise ValueError(f"Unknown method: {method}. Use 'fft', 'periodogram', 'welch', or 'multitaper'")
    
    return frequencies, power


def blackman_tukey(data: np.ndarray,
...
def coherence(x: np.ndarray, y: np.ndarray,
              dt: float = 1.0,
              window_size: int = 256,
...
def cross_spectrum(x: np.ndarray, y: np.ndarray,
                   dt: float = 1.0,
                   window_size: int = 256,
                   overlap: int = 128) -> Tuple[np.ndarray, np.ndarray]:
    """
...
def evolutionary_spectrum(data: np.ndarray,
                         sampling_rate: float = 1.0,
                         window_size: int = 256,
...
def multitaper_spectrum(data: np.ndarray,
                       dt: float = 1.0,
                       NW: float = 4.0,
                       k: Optional[int] = None,
...
def multitaper_coherence(x: np.ndarray, y: np.ndarray,
                         dt: float = 1.0,
                         NW: float = 4.0,
...
def multitaper_spectrogram(data: np.ndarray,
                           dt: float = 1.0,
