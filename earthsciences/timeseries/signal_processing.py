...
def convolve(signal1: np.ndarray, signal2: np.ndarray,
            mode: str = 'full',
...
def deconvolve(signal_data: np.ndarray,
...
def impulse_response(b: np.ndarray, a: np.ndarray,
...
def frequency_response(b: np.ndarray, a: np.ndarray,
                      sampling_rate: float = 1.0,
                      n_points: int = 512) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
...
def design_iir_filter(cutoff: Union[float, Tuple[float, float]],
                     sampling_rate: float,
                     filter_type: str = 'lowpass',
...
def transfer_function(b: np.ndarray, a: np.ndarray,
...
def group_delay(b: np.ndarray, a: np.ndarray,
               sampling_rate: float = 1.0,
