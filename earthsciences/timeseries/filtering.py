...
def fir_filter(
    ...
    if filter_type == "bandpass":
        taps = signal.firwin(num_taps, normalized_cutoff, pass_zero=False, window=window)
    elif filter_type == "bandstop":
        taps = signal.firwin(num_taps, normalized_cutoff, pass_zero=True, window=window)
    else:
        pass_zero = filter_type == "lowpass"
        taps = signal.firwin(num_taps, normalized_cutoff, pass_zero=pass_zero, window=window)
...
def moving_average_filter(data: np.ndarray, window_size: int = 5, mode: str = "same") -> np.ndarray:
    """
...
def median_filter(data: np.ndarray, kernel_size: int = 3, window: int | None = None) -> np.ndarray:
    """
    Apply median filter.
...
def notch_filter(
    data: np.ndarray, frequency: float, sampling_rate: float = 1.0, quality: float = 30.0
) -> np.ndarray:
    """
...
def bandpass_filter(
    data: np.ndarray, lowcut: float, highcut: float, sampling_rate: float = 1.0, order: int = 4
) -> np.ndarray:
    """
    ...
    return butter_filter(
        data, (lowcut, highcut), sampling_rate, filter_type="bandpass", order=order
    )


def highpass_filter(
    data: np.ndarray, cutoff: float, sampling_rate: float = 1.0, order: int = 4
) -> np.ndarray:
    """
    ...
    ...                            sampling_rate=1000)
    """
    return butter_filter(data, cutoff, sampling_rate, filter_type="highpass", order=order)
...
def lowpass_filter(
    ...
    ...                           sampling_rate=1000)
    """
    return butter_filter(data, cutoff, sampling_rate, filter_type="lowpass", order=order)
...
def wiener_filter(data: np.ndarray, noise_power: float | None = None) -> np.ndarray:
    """
