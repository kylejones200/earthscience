...
def stacking_velocity_analysis(gathers: np.ndarray, offsets: np.ndarray,
                              times: np.ndarray,
                              velocity_range: Tuple[float, float] = (1500, 4000),
                              n_velocities: int = 50) -> Dict:
    """
    ...
    for i, t0 in enumerate(times):
        for j, v in enumerate(velocities):
            ...
            time_indices = np.clip((t_nmo / times[-1] * (len(times) - 1)).astype(int), 0, len(times) - 1)
...
def eikonal_traveltime(velocity_model: np.ndarray, source_x: int, source_y: int,
