...
def magnetic_anomaly_sphere(x: np.ndarray, y: np.ndarray, z: np.ndarray,
                           x0: float, y0: float, z0: float, radius: float,
                           susceptibility: float, inclination: float,
                           declination: float, field_strength: float = 50000) -> np.ndarray:
    """
    ...
    anomaly = (M / r**3) * (3 * dot_product * (l * dx + m * dy + n * dz) / r**2 
                            - (l**2 + m**2 + n**2))
...
def reduction_to_pole(magnetic_data: np.ndarray, inclination: float,
