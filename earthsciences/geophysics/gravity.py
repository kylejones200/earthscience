...
def bouguer_correction(
    observed_gravity: np.ndarray, elevation: np.ndarray, latitude: np.ndarray, density: float = 2670
) -> dict:
    """
    ...
    normal_gravity = 978032.7 * (
        1 + 0.0053024 * np.sin(lat_rad) ** 2 - 0.0000058 * np.sin(2 * lat_rad) ** 2
    )
...
def terrain_correction(
    ...
    terrain_effect = (
        0.04193 * (density / 1000) * np.abs(terrain_diff) * 0.1
    )  # Factor for terrain irregularity
