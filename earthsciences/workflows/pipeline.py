...
class GeostatsPipeline:
    ...
    def load_data(self):
        ...
        print(f"✓ Loaded {len(x)} data points from {input_file.name}")
    ...
    def preprocess(self):
        ...
        if self.config.preprocessing.remove_outliers:
            ...
            if n_outliers > 0:
                ...
                z = self.data['z']
                print(f"✓ Removed {n_outliers} outliers using {method} method")
                print(f"  Retained {len(z)} / {n_original} points ({100*len(z)/n_original:.1f}%)")
            ...
            if transform_name == 'log':
                if np.any(z <= 0):
                    if self.config.preprocessing.handle_negatives:
                        ...
                        print(f"✓ Applied log transform (shifted by {shift:.6f})")
                ...
                if np.any(z <= 0):
                    ...
                    print(f"✓ Applied Box-Cox transform (λ={lambda_param:.3f}, shift={shift:.6f})")
                    self.transformer = {'type': 'boxcox', 'lambda': lambda_param, 'shift': shift}
                else:
                    z_transformed, lambda_param = boxcox(z)
                    print(f"✓ Applied Box-Cox transform (λ={lambda_param:.3f})")
    ...
    def fit_variogram(self):
        ...
        print(f"✓ Computed experimental variogram")
        print(f"  Lags: {len(lags)}")
        print(f"  Max lag: {lags[-1]:.2f}")
        print(f"  Sill (approximate): {gamma[-1]:.4f}")
        ...
        if self.config.variogram.auto_fit:
            ...
            print(f"✓ Best model: {best_model['model_name'].capitalize()}")
            print(f"  Nugget: {best_model.get('nugget', 0):.4f}")
            print(f"  Sill: {best_model.get('sill', 0):.4f}")
            print(f"  Range: {best_model.get('range', 0):.2f}")
            print(f"  R²: {best_model.get('r_squared', 0):.4f}")
        ...
        self.results['variogram'] = {
            ...
            } if self.variogram_model else None
        }
    
    def krige(self):
        """Perform kriging interpolation."""
        print(f"\n{'='*70}")
        print("STEP 4: KRIGING INTERPOLATION")
        print(f"{'='*70}")
        
        from earthsciences.spatial import ordinary_kriging
        from earthsciences.spatial.variogram import (
            spherical_model, exponential_model, gaussian_model
        )
        ...
        print(f"  Grid size: {X.shape[1]} × {X.shape[0]}")
        ...
        print(f"  Prediction range: [{Z_pred_orig.min():.2f}, {Z_pred_orig.max():.2f}]")
        print(f"  Mean prediction: {Z_pred_orig.mean():.2f}")
    ...
    def _plot_variogram(self, output_dir, use_clean_style):
        ...
        ax.plot(self.variogram_data['lags'], self.variogram_data['gamma'], 
               'o', color='#1976D2', markersize=8, label='Experimental', 
        ...
        if self.variogram_model:
            ...
            ax.plot(lags_fine, gamma_model, '-', color='#D32F2F', linewidth=2.5,
                   label=f'{model_name.capitalize()} model')
        
        ax.set_title(f'Variogram: Semivariance vs Lag Distance', 
        ...
        print(f"  ✓ Created variogram plot")
    ...
    def _plot_variance_map(self, output_dir, use_clean_style):
        ...
        ax.set_title('Kriging Variance (Uncertainty) Map',
                    fontsize=13, fontweight='bold', pad=15)
    ...
    def _plot_cross_validation(self, output_dir, use_clean_style):
        ...
        ax.scatter(y_true, y_pred, alpha=0.6, s=40, color='#1976D2',
                  edgecolors='black', linewidth=0.5)
        
        # 1:1 line
        lims = [min(y_true.min(), y_pred.min()), max(y_true.max(), y_pred.max())]
        ax.plot(lims, lims, '--', color='#D32F2F', linewidth=2.5, alpha=0.7,
               label='1:1 line')
        ...
        ax.set_title('Cross-Validation: Predicted vs Actual Values',
                    fontsize=13, fontweight='bold', pad=15)
        ax.legend(frameon=False, fontsize=11)
        ...
        print(f"  ✓ Created cross-validation plot")
        self.plots.append(str(output_path))
    ...
    def _plot_histogram(self, output_dir, use_clean_style):
        ...
        ax.hist(self.data['z'], bins=30, alpha=0.8, edgecolor='black',
               linewidth=0.8, color='#64B5F6')
        
        mean_val = np.mean(self.data['z'])
        median_val = np.median(self.data['z'])
        
        ax.axvline(mean_val, color='#D32F2F', linestyle='--', linewidth=2.5,
                  label=f'Mean: {mean_val:.2f}')
        ax.axvline(median_val, color='#388E3C', linestyle='--', linewidth=2.5,
                  label=f'Median: {median_val:.2f}')
        
        ax.set_title(f'Data Distribution: {self.config.data.z_column}',
                    fontsize=13, fontweight='bold', pad=15)
        ax.legend(frameon=False, fontsize=11)
    ...
    def save_results(self):
        ...
        if self.config.output.save_predictions and self.kriging_result:
            ...
            if 'npy' in self.config.output.formats:
                ...
                print(f"  ✓ Saved predictions to NPY")
