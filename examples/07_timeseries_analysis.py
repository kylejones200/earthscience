...
# =============================================================================
print("\n" + "=" * 70)
print("Example 1: Seismic Signal Filtering")
print("=" * 70)
...
print(f"\nFilters applied:")
print(f"  Lowpass (< 3 Hz): Isolates surface waves")
print(f"  Highpass (> 5 Hz): Isolates body waves")
print(f"  Bandpass (0.5-2 Hz): Targets Love/Rayleigh waves")
...
axes[0].set_title('Synthetic Seismic Signal: Surface + Body Waves + Noise', 
                 fontsize=12, fontweight='bold', pad=12)
clean_plot_style(axes[0])

# Lowpass filtered
axes[1].plot(t, lowpass, '#1976D2', linewidth=1.5)
axes[1].set_title('Lowpass Filtered < 3 Hz (Surface Waves Isolated)', 
                 fontsize=12, fontweight='bold', pad=12)
clean_plot_style(axes[1])

# Highpass filtered
axes[2].plot(t, highpass, '#D32F2F', linewidth=1.5)
axes[2].set_title('Highpass Filtered > 5 Hz (Body Waves Isolated)', 
                 fontsize=12, fontweight='bold', pad=12)
clean_plot_style(axes[2])

# Bandpass filtered
axes[3].plot(t, bandpass, '#388E3C', linewidth=1.5)
axes[3].set_title('Bandpass Filtered 0.5-2 Hz (Love/Rayleigh Waves)', 
                 fontsize=12, fontweight='bold', pad=12)
clean_plot_style(axes[3])


plt.tight_layout()
plt.savefig('07_timeseries_filtering.png', dpi=300, bbox_inches='tight')
...
# =============================================================================
print("\n" + "=" * 70)
print("Example 2: Spectral Analysis of Climate Data")
print("=" * 70)
...
print(f"  Resolution: monthly")
print(f"  Data points: {len(t_climate)}")
...
print(f"\nSpectral analysis:")
print(f"  Method 1: Welch periodogram")
print(f"  Method 2: Multitaper spectrum (optimal for line spectra)")
...
ax1.plot(t_climate, trend, '#D32F2F', linestyle='--', linewidth=2.5, 
        label='Linear trend', alpha=0.8)
ax1.set_title('Simulated Climate Anomaly (°C) Over 100 Years (Monthly Data)', 
...
# Power spectrum
ax2.loglog(periods_welch, power_welch[1:], '#1976D2', alpha=0.6, 
          linewidth=2, label='Welch method')
ax2.loglog(periods_mt, power_mt[1:], '#D32F2F', linewidth=2.5, 
          label='Multitaper method')

# Mark expected peaks
for period, label in [(1, 'Annual'), (4, 'ENSO'), (20, 'Decadal')]:
    ax2.axvline(period, color='#388E3C', linestyle='--', alpha=0.7, linewidth=2)
    ax2.text(period*1.1, ax2.get_ylim()[1]*0.7, label, rotation=0, 
            va='center', ha='left', fontsize=10, color='#388E3C')

ax2.set_title('Power Spectrum: Power vs Period (years)', 
             fontsize=13, fontweight='bold', pad=15)
...
plt.tight_layout()
plt.savefig('07_timeseries_spectral.png', dpi=300, bbox_inches='tight')
...
# =============================================================================
print("\n" + "=" * 70)
print("Example 3: Wavelet Analysis - Time-Frequency Decomposition")
print("=" * 70)
...
print(f"  Chirp: 5 → 20 Hz over 10 seconds")
print(f"  Transient: 15 Hz burst at t=5s")
print(f"  Sampling rate: {fs_wavelet} Hz")

# Continuous Wavelet Transform
scales = np.arange(1, 128)
cwt_result = cwt(wavelet_signal, wavelet='morl', scales=scales, 
                 sampling_period=1/fs_wavelet)
...
ax1.set_title('Non-Stationary Signal: Chirp (Increasing Frequency) + Transient Pulse', 
             fontsize=13, fontweight='bold', pad=15)
ax1.set_xlim(0, 10)
clean_plot_style(ax1)
...
ax2.set_title('Wavelet Scalogram: Frequency (Hz) vs Time (seconds)', 
             fontsize=13, fontweight='bold', pad=15)
ax2.set_ylim(0, 25)
...
plt.tight_layout()
plt.savefig('07_timeseries_wavelet.png', dpi=300, bbox_inches='tight')
...
# =============================================================================
print("\n" + "=" * 70)
print("Example 4: Lomb-Scargle for Irregular Sampling")
print("=" * 70)
...
np.random.seed(42)
keep_fraction = 0.6
keep_indices = np.random.choice(len(t_regular), int(len(t_regular) * keep_fraction), 
                               replace=False)
t_irregular = np.sort(t_regular[keep_indices])

# Signal with two periodicities
true_periods = [10, 25]  # days
signal_irregular = (np.sin(2 * np.pi * t_irregular / true_periods[0]) +
                   0.7 * np.sin(2 * np.pi * t_irregular / true_periods[1]) +
                   np.random.normal(0, 0.3, len(t_irregular)))

print(f"\nIrregularly sampled data:")
print(f"  Total timespan: 100 days")
print(f"  Regular grid: 500 points")
print(f"  Irregular sample: {len(t_irregular)} points ({keep_fraction*100:.0f}%)")
print(f"  True periods: {true_periods} days")

# Lomb-Scargle periodogram
frequencies = np.linspace(0.01, 0.5, 1000)  # 0.01 to 0.5 cycles/day
freqs_ls, power_ls = lomb_scargle(t_irregular, signal_irregular,
...
for i, period in enumerate(sorted(detected_periods, reverse=True)):
    print(f"  Peak {i+1}: {period:.1f} days")

# Plot
fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(14, 10))

# Time series
ax1.plot(t_irregular, signal_irregular, 'o', color='#1976D2', markersize=5, 
        alpha=0.7, markeredgewidth=0)
ax1.set_title('Irregularly Sampled Time Series (60% of Regular Grid)', 
             fontsize=13, fontweight='bold', pad=15)
clean_plot_style(ax1)

# Lomb-Scargle periodogram
ax2.plot(periods_ls, power_ls, '#1976D2', linewidth=2)
ax2.plot(periods_ls[peaks], power_ls[peaks], 'o', color='#D32F2F', markersize=12,
        label='Detected peaks', zorder=5, markeredgecolor='black', markeredgewidth=1)

# Mark true periods
for period in true_periods:
    ax2.axvline(period, color='#388E3C', linestyle='--', linewidth=2.5, alpha=0.7)
    ax2.text(period, ax2.get_ylim()[1]*0.9, f'{period}d true', 
            ha='center', fontsize=10, fontweight='bold', color='#388E3C')

ax2.set_title('Lomb-Scargle Periodogram: Power vs Period (days)', 
             fontsize=13, fontweight='bold', pad=15)
ax2.legend(frameon=False, fontsize=11)
ax2.set_xlim(2, 50)
clean_plot_style(ax2)
...
plt.tight_layout()
plt.savefig('07_timeseries_lombscargle.png', dpi=300, bbox_inches='tight')
