...
def rb_sr_isochron_age(rb_sr_ratios, sr_ratios, uncertainties):
    ...
    return isochron_dating(rb_sr_ratios, sr_ratios, uncertainties, DECAY_CONSTANTS["Rb87"])


log_section("GEOCHRONOLOGY EXAMPLES")

# =============================================================================
# Example 1: Radioactive Decay Basics
# =============================================================================
log_section("Example 1: Radioactive Decay")
...
logger.info(f"  Initial atoms: {N0}")
logger.info(f"  Time elapsed: {t/1e9:.1f} Ga")
logger.info(f"  Remaining: {N_remaining:.1f} atoms ({percent_remaining:.1f}%)")
logger.info(f"  Decayed: {N0 - N_remaining:.1f} atoms")
logger.info(f"  Half-life: {np.log(2)/lambda_238/1e9:.2f} Ga")
...
ax1.set_title(
    "Percent Remaining of Common Radioisotopes Over 10 Billion Years",
    fontsize=13,
    fontweight="bold",
    pad=15,
)
...
ax2.set_title(
    "Radioactive Decay on Logarithmic Scale (% Remaining vs Time in Ga)",
    fontsize=13,
    fontweight="bold",
    pad=15,
)
...
plt.tight_layout()
plt.savefig(TUTORIAL_DIR / "05_geochronology_decay.png", dpi=300, bbox_inches="tight")
...
# =============================================================================
log_section("Example 2: U-Pb Dating")
...
logger.info("\nZircon U-Pb dating:")
logger.info(f"  206Pb/238U = {pb206_u238_ratio:.4f} → Age = {age_206_238/1e6:.1f} Ma")
logger.info(f"  207Pb/235U = {pb207_u235_ratio:.4f} → Age = {age_207_235/1e6:.1f} Ma")

# Check concordance
discordance = abs(age_206_238 - age_207_235) / age_206_238 * 100
logger.info(f"  Discordance: {discordance:.1f}%")

if discordance < 5:
    logger.info(f"  ✓ Concordant age: {age_206_238/1e6:.1f} Ma")
else:
    logger.info("  ⚠ Discordant - possible Pb loss")

# =============================================================================
# Example 3: Concordia Diagram
# =============================================================================
log_section("Example 3: Concordia Diagram")
...
ax.plot(pb207_u235_curve, pb206_u238_curve, "k-", linewidth=2.5, label="Concordia", zorder=1)
...
for age_ma in [100, 250, 500, 1000, 2000, 3000, 4000]:
    ...
    ax.annotate(f"{age_ma}", (x, y), xytext=(5, 5), textcoords="offset points", fontsize=9)
...
ax.scatter(
    ...
    edgecolors="black",
    linewidth=1.5,
    label="Discordant (Pb loss)",
    zorder=3,
    alpha=0.9,
)

ax.set_title(
    "Zircon U-Pb Concordia Diagram: ²⁰⁶Pb/²³⁸U vs ²⁰⁷Pb/²³⁵U (Ages in Ma)",
    fontsize=14,
    fontweight="bold",
    pad=20,
)
...
plt.tight_layout()
plt.savefig(TUTORIAL_DIR / "05_geochronology_concordia.png", dpi=300, bbox_inches="tight")
logger.info("\n✓ Saved concordia diagram")
plt.close()

logger.info("\nConcordia analysis:")
logger.info(f"  True age: {true_age/1e6:.0f} Ma")
logger.info(
    f"  Concordant points: {len(pb206_u238_conc)} (mean age: {np.mean(ages_concordant)/1e6:.0f} Ma)"
)
logger.info(f"  Discordant points: {len(pb206_u238_disc)} (apparent ages too young)")

# =============================================================================
# Example 4: Rb-Sr Isochron Dating
# =============================================================================
log_section("Example 4: Rb-Sr Isochron Dating")
...
logger.info(f"  True age: {true_age_rbsr/1e6:.0f} Ma")
logger.info(f"  Calculated age: {result['age']/1e6:.0f} Ma")
logger.info(f"  Initial ⁸⁷Sr/⁸⁶Sr: {result['initial_ratio']:.6f} (true: {initial_ratio:.6f})")
logger.info(f"  MSWD: {result['mswd']:.2f} (good fit if ~1)")
...
ax.fill_between(
    x_line, y_line - error_estimate, y_line + error_estimate, alpha=0.15, color="#D32F2F"
)

ax.set_title(
    "Rb-Sr Isochron: ⁸⁷Sr/⁸⁶Sr vs ⁸⁷Rb/⁸⁶Sr for Granite Whole Rock Samples",
    fontsize=14,
    fontweight="bold",
    pad=20,
)
...
plt.tight_layout()
plt.savefig(TUTORIAL_DIR / "05_geochronology_isochron.png", dpi=300, bbox_inches="tight")
logger.info("\n✓ Saved isochron plot")
plt.close()
...
# =============================================================================
log_section("Example 5: Radiocarbon Dating")
...
logger.info("\nRadiocarbon ages:")
logger.info(f"{'Sample':<30} {'¹⁴C/¹²C':<12} {'Age (BP)':<20} {'Cal. Age (BP)':<15}")
logger.info("-" * 80)
...
ax1.axhline(y=1, color="#D32F2F", linestyle="--", linewidth=2, alpha=0.7, label="Detection limit")
...
ax1.set_title(
    "Radiocarbon Decay: Percent Modern Carbon Over Time (Years BP)",
    fontsize=13,
    fontweight="bold",
    pad=15,
)
...
ax2.set_title(
    "Radiocarbon Age (ka BP) vs Percent Modern Carbon", fontsize=13, fontweight="bold", pad=15
)
...
plt.tight_layout()
plt.savefig(TUTORIAL_DIR / "05_geochronology_radiocarbon.png", dpi=300, bbox_inches="tight")
...
logger.info("✓ All geochronology examples complete!")
