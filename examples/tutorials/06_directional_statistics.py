...
log_section("DIRECTIONAL STATISTICS EXAMPLES")

# =============================================================================
# Example 1: Structural Geology - Joint Orientations
# =============================================================================
log_section("Example 1: Joint Orientation Analysis")
...
logger.info(f"  Mean direction: {mean_direction:.1f}°")
logger.info(f"  Circular variance: {circ_var:.3f}")
logger.info(f"  Circular std dev: {circ_std:.1f}°")
logger.info(f"  Resultant length: {r_length:.3f} (1.0 = perfect alignment)")

# Rayleigh test for uniformity
rayleigh_result = rayleigh_test(all_joints, degrees=True)
logger.info("\nRayleigh test for uniformity:")
logger.info(f"  Test statistic: {rayleigh_result['statistic']:.4f}")
logger.info(f"  P-value: {rayleigh_result['p_value']:.6f}")
if rayleigh_result["p_value"] < 0.05:
    logger.info("  ✓ Non-uniform distribution (preferred orientation)")
else:
    logger.info("  Random distribution")
...
logger.info(f"  Mean direction: {fit1['mu']:.1f}°")
logger.info(f"  Concentration (κ): {fit1['kappa']:.2f}")

logger.info(f"\nJoint Set 2 (n={len(joints_set2)}):")
logger.info(f"  Mean direction: {fit2['mu']:.1f}°")
logger.info(f"  Concentration (κ): {fit2['kappa']:.2f}")
...
# Plot bars
bars = ax1.bar(
    bin_centers, counts, width=np.deg2rad(10), bottom=0, alpha=0.7, edgecolor="black", linewidth=1
)
...
ax1.set_title(
    "Rose Diagram of All Joint Strike Azimuths (n=45)", fontsize=12, fontweight="bold", pad=20
)
...
bars2 = ax2.bar(
    ...
    linewidth=1,
    bottom=counts1,
    label=f'Set 2: {fit2["mu"]:.1f}°',
)

ax2.set_theta_zero_location("N")
ax2.set_theta_direction(-1)
ax2.set_title(
    "Rose Diagram Showing Two Distinct Joint Sets", fontsize=12, fontweight="bold", pad=20
)
ax2.legend(loc="upper right", bbox_to_anchor=(1.3, 1.1), frameon=False)
ax2.spines["polar"].set_linewidth(0.8)

plt.tight_layout()
plt.savefig(TUTORIAL_DIR / "06_directional_joints.png", dpi=300, bbox_inches="tight")
...
# =============================================================================
log_section("Example 2: Paleomagnetic Directions")
...
logger.info(f"  True direction: Dec={true_dec:.1f}°, Inc={true_inc:.1f}°")
logger.info(f"  Mean direction: Dec={mean_dec_circ:.1f}°, Inc={mean_inc_avg:.1f}°")
logger.info(f"  Fisher κ: {k:.1f}")
logger.info(f"  α₉₅ (95% confidence): {140 / np.sqrt(k):.1f}°")

# Rayleigh test for significant clustering
rayleigh_result = rayleigh_test(decs, degrees=True)
logger.info("\nRayleigh test for clustering:")
logger.info(f"  Test statistic: {rayleigh_result['statistic']:.4f}")
logger.info(f"  P-value: {rayleigh_result['p_value']:.4f}")
if rayleigh_result["p_value"] < 0.05:
    logger.info("  ✓ Significant preferred direction")
...
ax1.axvline(true_dec, color="#388E3C", linestyle=":", linewidth=2.5, label=f"True: {true_dec:.1f}°")
ax1.set_title(
    "Paleomagnetic Declination Distribution (n=30 samples)", fontsize=13, fontweight="bold", pad=15
)
...
ax2.axvline(
    mean_inc_avg, color="#D32F2F", linestyle="--", linewidth=2.5, label=f"Mean: {mean_inc_avg:.1f}°"
)
ax2.axvline(true_inc, color="#388E3C", linestyle=":", linewidth=2.5, label=f"True: {true_inc:.1f}°")
ax2.set_title(
    "Paleomagnetic Inclination Distribution (n=30 samples)", fontsize=13, fontweight="bold", pad=15
)
ax2.legend(frameon=False, fontsize=10)
clean_plot_style(ax2)

plt.tight_layout()
plt.savefig(TUTORIAL_DIR / "06_directional_paleomag.png", dpi=300, bbox_inches="tight")
...
# =============================================================================
log_section("Example 3: Wind Direction Analysis")
...
logger.info(f"  Summer mean: {circular_mean(summer_winds, degrees=True):.1f}°")
logger.info(f"  Winter mean: {circular_mean(winter_winds, degrees=True):.1f}°")
logger.info("\nKuiper test (comparing seasons):")
logger.info(f"  Test statistic: {kuiper_result['statistic']:.4f}")
logger.info(f"  P-value: {kuiper_result['p_value']:.6f}")
if kuiper_result["p_value"] < 0.05:
    logger.info("  ✓ Significantly different distributions")
...
# Summer
ax1 = axes[0]
counts_summer, edges_summer = np.histogram(
    np.deg2rad(summer_winds), bins=np.linspace(0, 2 * np.pi, 37)
)
...
ax1.plot(
    [np.deg2rad(mean_summer), np.deg2rad(mean_summer)],
    [0, max(counts_summer) * 1.1],
    "r-",
    linewidth=3,
...
# Winter
ax2 = axes[1]
counts_winter, edges_winter = np.histogram(
    np.deg2rad(winter_winds), bins=np.linspace(0, 2 * np.pi, 37)
)
...
ax2.plot(
    [np.deg2rad(mean_winter), np.deg2rad(mean_winter)],
    [0, max(counts_winter) * 1.1],
    "r-",
    linewidth=3,
...
plt.tight_layout()
plt.savefig(TUTORIAL_DIR / "06_directional_winds.png", dpi=300, bbox_inches="tight")
...
# =============================================================================
log_section("Example 4: Fault Slip Vector Analysis")
...
logger.info("\nNormal faulting:")
logger.info(f"  Mean rake: {circular_mean(normal_fault_rakes, degrees=True):.1f}°")
logger.info(f"  Circular std: {circular_std(normal_fault_rakes, degrees=True):.1f}°")
logger.info("  → Dip-slip dominated (rake ~90°)")

logger.info("\nStrike-slip faulting:")
logger.info(f"  Mean rake: {circular_mean(strike_slip_rakes, degrees=True):.1f}°")
logger.info(f"  Circular std: {circular_std(strike_slip_rakes, degrees=True):.1f}°")
logger.info("  → Strike-slip dominated (rake ~0° or 180°)")

# Plot comparison
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 5))

# Normal fault rakes
ax1.hist(normal_fault_rakes, bins=20, alpha=0.8, edgecolor="black", linewidth=0.8, color="#E53935")
ax1.axvline(90, color="#1976D2", linestyle="--", linewidth=2.5, label="Pure dip-slip (90°)")
ax1.axvline(
    circular_mean(normal_fault_rakes, degrees=True),
    color="#000000",
    linestyle="-",
    linewidth=2.5,
...
ax1.set_title(
    "Slickenline Rake Angles on Normal Faults (Dip-Slip Dominated)",
    fontsize=13,
    fontweight="bold",
    pad=15,
)
...
# Strike-slip rakes
ax2.hist(strike_slip_rakes, bins=20, alpha=0.8, edgecolor="black", linewidth=0.8, color="#43A047")
ax2.axvline(0, color="#1976D2", linestyle="--", linewidth=2.5, label="Pure strike-slip")
ax2.axvline(180, color="#1976D2", linestyle="--", linewidth=2.5)
ax2.axvline(
    circular_mean(strike_slip_rakes, degrees=True),
    color="#000000",
    linestyle="-",
    linewidth=2.5,
...
ax2.set_title(
    "Slickenline Rake Angles on Strike-Slip Faults", fontsize=13, fontweight="bold", pad=15
)
ax2.legend(frameon=False, fontsize=10)
ax2.set_xlim(0, 180)
clean_plot_style(ax2)
...
plt.tight_layout()
plt.savefig(TUTORIAL_DIR / "06_directional_faults.png", dpi=300, bbox_inches="tight")
...
logger.info("✓ All directional statistics examples complete!")
