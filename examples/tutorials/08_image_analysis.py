...
log_section("IMAGE ANALYSIS EXAMPLES")

# =============================================================================
# Example 1: Vegetation Indices from Satellite Imagery
# =============================================================================
log_section("Example 1: Vegetation Indices")
...
logger.info(f"  NDVI range: {ndvi_img.min():.3f} to {ndvi_img.max():.3f}")
logger.info(f"  EVI range: {evi_img.min():.3f} to {evi_img.max():.3f}")
logger.info(f"  SAVI range: {savi_img.min():.3f} to {savi_img.max():.3f}")

logger.info("\nTypical NDVI values:")
logger.info(f"  Water: {ndvi_img[water_mask].mean():.3f}")
logger.info(f"  Bare soil: {ndvi_img[soil_mask].mean():.3f}")
logger.info(f"  Grassland: {ndvi_img[grass_mask].mean():.3f}")
logger.info(f"  Forest: {ndvi_img[forest_mask].mean():.3f}")
...
plt.suptitle("Vegetation Indices from Satellite Imagery", fontsize=16, fontweight="bold", y=0.98)
plt.tight_layout()
plt.savefig(TUTORIAL_DIR / "08_image_vegetation_indices.png", dpi=300, bbox_inches="tight")
...
# =============================================================================
log_section("Example 2: Image Enhancement Techniques")
...
logger.info(f"  Min: {original_img.min():.3f}")
logger.info(f"  Max: {original_img.max():.3f}")
logger.info(f"  Mean: {original_img.mean():.3f}")
logger.info(f"  Std: {original_img.std():.3f}")
...
logger.info("  1. Histogram equalization - Enhances contrast globally")
logger.info("  2. Contrast stretch (2-98%) - Linear stretch of histogram")
logger.info("  3. Gamma correction (γ=0.7) - Brightens midtones")
logger.info("  4. Unsharp mask - Enhances edges and details")
...
axes[0, 1].set_title(
    "Intensity Distribution (Frequency vs Intensity)", fontsize=11, fontweight="bold"
)
...
plt.tight_layout()
plt.savefig(TUTORIAL_DIR / "08_image_enhancement.png", dpi=300, bbox_inches="tight")
...
# =============================================================================
log_section("Example 3: Grain Size Analysis")
...
# Analyze grain sizes
result = grain_size_distribution(
    grain_img, threshold_method="otsu", min_size=50, return_labels=True
)

logger.info("\nGrain size analysis:")
logger.info(f"  Number of grains: {result['n_grains']}")
logger.info(f"  Mean diameter: {result['mean_diameter']:.1f} pixels")
logger.info(f"  Median diameter: {result['median_diameter']:.1f} pixels")
logger.info(f"  Std deviation: {result['std_diameter']:.1f} pixels")
logger.info(f"  25th percentile: {result['percentiles']['25th']:.1f} pixels")
logger.info(f"  50th percentile: {result['percentiles']['50th']:.1f} pixels")
logger.info(f"  75th percentile: {result['percentiles']['75th']:.1f} pixels")

# Shape analysis
shape_result = shape_analysis(grain_img, min_size=50)

logger.info("\nShape analysis:")
logger.info(f"  Mean circularity: {shape_result['mean_circularity']:.3f}")
logger.info(f"  Mean elongation: {shape_result['mean_elongation']:.3f}")
if "solidity" in shape_result:
    logger.info(f"  Mean solidity: {np.mean(shape_result['solidity']):.3f}")
...
axes[0, 1].set_title(f'Labeled Grains (n={result["n_grains"]})', fontsize=12, fontweight="bold")
axes[0, 1].axis("off")

# Grain size distribution
axes[1, 0].hist(
    result["diameters"], bins=30, alpha=0.8, edgecolor="black", linewidth=0.8, color="#64B5F6"
)
axes[1, 0].axvline(
    ...
    linestyle="--",
    linewidth=2.5,
    label=f'Mean: {result["mean_diameter"]:.1f}',
)
axes[1, 0].axvline(
    result["median_diameter"],
    color="#388E3C",
    linestyle="--",
    linewidth=2.5,
    label=f'Median: {result["median_diameter"]:.1f}',
)
axes[1, 0].set_title(
    "Grain Size Distribution: Frequency vs Diameter (pixels)", fontsize=12, fontweight="bold"
)
...
data = [shape_result["circularity"], shape_result["elongation"], shape_result["solidity"]]
...
plt.suptitle("Automated Grain Analysis from Thin Section", fontsize=16, fontweight="bold", y=0.995)
plt.tight_layout()
plt.savefig(TUTORIAL_DIR / "08_image_grain_analysis.png", dpi=300, bbox_inches="tight")
...
logger.info("✓ All image analysis examples complete!")
