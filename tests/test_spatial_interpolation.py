...
class TestNaturalNeighbor:
    ...
    @pytest.mark.skipif(True, reason="Natural neighbor may require scipy.spatial.Voronoi")
    def test_basic_functionality(self):
        """Test basic natural neighbor interpolation."""
        pass
...
class TestRBFInterpolation:
    ...
    def test_different_kernels(self):
        ...
        for kernel in ['gaussian', 'multiquadric', 'thin_plate']:
            result = interpolation.rbf_interpolation(
                x, y, values, grid_x, grid_y, kernel=kernel
            )
            assert result.shape == grid_x.shape
